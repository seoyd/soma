...
use soma_zero::{SafeConsolidationPatchV2Config, SafeConsolidationPatchV2Runner};
...
fn config_defaults_are_safe_and_runner_builds_bundle() {
    let config = SafeConsolidationPatchV2Config::default();
    ...
    let bundle = run_sprint108(
        ...
        "safe-consolidation-patch-v2",
    );
    assert_eq!(
        bundle
            .second_safe_consolidation_patch_selection_report
            .selected_status,
        "SecondPatchCandidateSelected"
    );
    assert_eq!(
        bundle.safety_coverage_preservation_report_v24.safety_status,
...
fn remote_paths_are_rejected() {
    let config = SafeConsolidationPatchV2Config {
        sprint107_bundle_paths: Some(vec!["https://example.com/report.json".to_string()]),
        ..SafeConsolidationPatchV2Config::default()
    };
...
fn safety_coverage_missing_when_sentinel_preservation_is_disabled() {
    let mut config = sprint108_config_from_example(
        "soma_sprint108_safe_consolidation_patch_v2.toml",
        "safe-consolidation-patch-v2-missing-sentinel",
    );
...
fn assertion_preservation_is_required_for_verification_and_safety_v24() {
    let mut config = sprint108_config_from_example(
        "soma_sprint108_safe_consolidation_patch_v2.toml",
        "safe-consolidation-patch-v2-missing-assertion-preservation",
    );
...
#[test]
fn equivalent_coverage_is_required_before_retiring_second_target() {
    let mut config = sprint108_config_from_example(
        "soma_sprint108_safe_consolidation_patch_v2.toml",
        "safe-consolidation-patch-v2-missing-equivalent-coverage",
    );
