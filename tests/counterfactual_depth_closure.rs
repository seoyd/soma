...
use soma_zero::{
    ...
    CounterfactualDepthClosureStatus, ProviderMarket,
};

fn row(id: &str, complete: bool) -> ComparableCommitteeEvidenceRow {
    ComparableCommitteeEvidenceRow {
        ...
        baseline_action: complete.then(|| "Approve".to_string()),
        external_action: None,
        no_trade_baseline_action: "NoTrade".to_string(),
        outcome_label: complete.then(|| "TakeProfit".to_string()),
        ...
        outcome_reference_available: complete,
        baseline_reference_available: complete,
        no_trade_counterfactual_available: complete,
        risk_denied_counterfactual_available: complete,
        external_reference_available: false,
        row_level: complete,
        summary_derived: !complete,
...
#[test]
fn closure_reports_missing_baseline_without_improvement() {
    let comparable_config = ComparableCommitteeEvidenceConfig {
        comparable_id: "closure-base".to_string(),
        output_root: common::output_dir("closure-base-config")
            .display()
            .to_string(),
        ..ComparableCommitteeEvidenceConfig::default()
    };
    let before =
        ComparableCommitteeEvidenceBundle::from_rows(&comparable_config, vec![row("gap", false)]);
    let bundle_dir = common::output_dir("closure-before-bundle");
    let bundle_path = before.write_to_dir(&bundle_dir).expect("write bundle");

    let config = CounterfactualDepthClosureConfig {
        closure_id: "closure-status".to_string(),
        comparable_evidence_bundle_path: Some(bundle_path.display().to_string()),
        output_root: common::output_dir("closure-status-out")
            .display()
            .to_string(),
    ...
    let report = CounterfactualDepthClosureRunner::default()
        .run(&config)
        .expect("run closure");
    assert_eq!(
