mod support;

use support::sprint107_support::run_sprint107;

#[test]
fn sprint107_bundle_is_deterministic_and_summary_has_required_headings() {
    let mut left = run_sprint107(
        "soma_sprint107_safe_consolidation_patch.toml",
        "sprint107-determinism-left",
    );
    let mut right = run_sprint107(
        "soma_sprint107_safe_consolidation_patch.toml",
        "sprint107-determinism-right",
    );
    left.storage_report.output_dir = "<normalized>".to_string();
    right.storage_report.output_dir = "<normalized>".to_string();
    assert_eq!(
        left.to_json_string().expect("left json"),
        right.to_json_string().expect("right json")
    );
    for heading in [
        "## 1. Sprint summary",
        "## 24. Acceptance truth gate v8",
        "## 30. Control Tower safe consolidation patch panel",
        "## 47. Next gstack sprint recommendation",
    ] {
        assert!(
            left.final_summary.contains(heading),
            "missing heading {heading}"
        );
    }
}
