...
pub struct KRXCollectionClosureStorageReport {
...
pub struct KRXOfficialCollectionClosureBundle {
    pub auth_dry_run_report: KRXCollectionDryRunReport,
    pub collection_batch_plan: KRXCollectionBatchPlan,
    #[serde(default)]
    pub raw_response_archive_summary: Option<KRXRawResponseArchiveSummary>,
    #[serde(default)]
    pub schema_drift_report: Option<KRXResponseSchemaDriftReport>,
    pub canonical_batch_validation_report: KRXCanonicalBatchValidationReport,
    pub candle_sufficiency_report: KRXCandleSufficiencyReport,
    #[serde(default)]
    pub outcome_link_closure_report: Option<KRXOutcomeLinkClosureReport>,
    pub downstream_rerun_v2_summary: KRXDownstreamRerunV2Summary,
    pub collection_closure_report: KRXOfficialCollectionClosureReport,
    pub storage_report: KRXCollectionClosureStorageReport,
    pub final_summary: String,
    pub reason_codes: Vec<ReasonCode>,
}

impl KRXCollectionClosureStorageReport {
    ...
    pub fn build(
        ...
        Self {
            ...
            compaction_recommendation: if budget_exceeded {
                "reduce KRX scope or keep only the smallest bounded closure bundle; evidence is never deleted automatically".to_string()
            } else {
                "within bounded storage budget".to_string()
            },
...
impl KRXOfficialCollectionClosureBundle {
    pub fn to_json_string(&self) -> Result<String, String> {
        serde_json::to_string_pretty(self).map_err(|err| err.to_string())
    }
    ...
    pub fn write_to_dir(&self, output_dir: &Path) -> Result<(), String> {
        fs::create_dir_all(output_dir).map_err(|err| err.to_string())?;
        self.auth_dry_run_report.write_to_dir(output_dir)?;
        self.collection_batch_plan.write_to_dir(output_dir)?;
        if let Some(summary) = &self.raw_response_archive_summary {
            summary.write_to_dir(output_dir)?;
        }
        if let Some(report) = &self.schema_drift_report {
            report.write_to_dir(output_dir)?;
        }
        self.canonical_batch_validation_report
            .write_to_dir(output_dir)?;
        self.candle_sufficiency_report.write_to_dir(output_dir)?;
        if let Some(report) = &self.outcome_link_closure_report {
            report.write_to_dir(output_dir)?;
        }
        self.downstream_rerun_v2_summary.write_to_dir(output_dir)?;
        self.collection_closure_report.write_to_dir(output_dir)?;
        self.storage_report.write_to_dir(output_dir)?;
        fs::write(
            output_dir.join("krx_collection_closure_summary.txt"),
            &self.final_summary,
        )
        .map_err(|err| err.to_string())?;
        fs::write(
            output_dir.join("krx_collection_closure_bundle.json"),
            self.to_json_string()?,
        )
        .map_err(|err| err.to_string())?;
        Ok(())
