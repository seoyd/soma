...
pub struct KRXActivationStorageReport {
...
impl KRXActivationStorageReport {
    pub fn build(
        ...
        let total_bytes = canonical_csv_bytes
            + raw_archive_bytes
            + provenance_bytes
            + preflight_bytes
            + manifest_bytes
            ...
            + report_bytes;
        let mut largest_artifacts = canonical_paths
            .iter()
            .chain(raw_archive_paths.iter())
        ...
        let mut reason_codes = vec![ReasonCode::KRXActivationStorageReportBuilt];
        if budget_exceeded {
