...
The pinned checkout is clean at the recorded commit. The developer pack verifies the official origin, commit, clean status, and SHA-256 values for `mamba3.py`, the CuTe step implementation, the BC normalization implementation, and the rotary step implementation before it imports the official module. No faithful CPU path was found in the inspected Mamba-3 module: the SISO combined and step paths import Triton/CUDA code, while `Mamba3.step` requires the CuTe step function and documents H100 testing. The selected local result is therefore `OfficialOracleExecutionBlocked`, not an output or state parity claim.

`tools/mamba3_reference/verify_environment.py` emits a machine-readable pre-flight result. It checks the selected CUDA device, H100 capability requirement, official dependency imports, official CuTe route, F32/BF16 selection, writable fixture output, and a finite deterministic CUDA allocation. `run_oracle_pack.py` only continues on `ReadyF32`; it uses explicit local environment variables or command arguments and has no fixed repository or home path. Oracle execution remains developer-only and does not change `backend-cuda` readiness.
